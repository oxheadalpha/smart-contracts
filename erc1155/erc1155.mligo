...
type erc1155 =
  (*
    ...
    MUST call `On_erc1155_received` on `to_` and act appropriately (see "Safe Transfer Rules"
  ...
  *)
  ...
  (*
    Transfers specified `amount`(s) of `token_id`(s) from the `from` address to the `to_`
    address specified (with safety call).
    Caller must be approved to manage the tokens being transferred out of the `from` account
    (see "Approval" section of the standard).
    MUST revert if any of the balance(s) of the holder(s) for token(s) is lower than the 
    respective amount(s) in `_values` sent to the recipient.
    MUST call the relevant `erc1155_token_receiver` hook(s) on `to_` and act appropriately 
