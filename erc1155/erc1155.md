...
requires that safety check MUST be performed only when target contract implements
`ERC1155TokenReceiver` interface. Otherwise, safety check is skipped.
...
the transaction MUST skip safety check. `data` argument must be passed unaltered
