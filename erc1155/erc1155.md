...
#### `ERC1155TokenReceiver` Rules

If an implementation specific API function is used to transfer ERC-1155 token(s) to a contract,
the `SafeTransferFrom`or `SafeBatchTransferFrom` (as appropriate) rules MUST still be followed
and the receiver MUST implement the `ERC1155TokenReceiver` interface.

Only non-standard transfer functions MAY allow tokens to be sent to a recipient contract that
does NOT implement the necessary ERC1155TokenReceiver hook functions.
