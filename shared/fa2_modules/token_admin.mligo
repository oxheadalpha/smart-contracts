...
type token_admin =
  ...
  | Pause of token_id list
  | Unpause of token_id list
...
let pause (tokens, s: (token_id list) * token_admin_storage) : token_admin_storage =
  let new_paused = List.fold 
    (fun (paused, t : paused_tokens_set * token_id) -> Big_map.add t unit paused)
    tokens s.paused in
  { s with paused = new_paused; }

let unpause (tokens, s: (token_id list) * token_admin_storage) : token_admin_storage =
  let new_paused = List.fold 
    (fun (paused, t : paused_tokens_set * token_id) -> Big_map.remove t paused)
    tokens s.paused in
  { s with paused = new_paused; }
